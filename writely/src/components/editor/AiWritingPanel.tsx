...
import captureAiContext from "~/features/editor/utils/aiContext/captureAiContext";
import isAiContextCurrent from "~/features/editor/utils/aiContext/isAiContextCurrent";
import replaceAiContext from "~/features/editor/utils/aiContext/replaceAiContext";
import countWords from "~/features/editor/utils/editorContent/countWords";
  ...
  return (
    <>
      <button
        ...
        tabIndex={-1}
        className={`fixed inset-0 z-40 cursor-default bg-black/80 transition-opacity duration-300 ease-out motion-reduce:transition-none ${
      ...
      <aside
        ...
        aria-hidden={!isOpen}
        inert={!isOpen}
        className={`fixed inset-y-0 right-0 z-50 flex h-dvh w-[min(100%,440px)] transform-gpu flex-col overflow-hidden border-l border-(--w-foreground) bg-(--w-background) transition-transform duration-300 ease-out will-change-transform motion-reduce:transition-none ${
        ...
        <div className="flex h-full min-h-0 flex-col">
          <div className="flex shrink-0 items-start justify-between gap-4 border-b border-(--w-foreground) px-6 py-5">
            <div>
              ...
              <p className="mt-1.5 text-xs text-(--w-muted)">
                {hasTarget
                  ? `Selected text · ${selectionWordCount.toLocaleString("en")} ${
          ...
          <div className="ai-panel-scrollbar min-h-0 flex-1 overflow-y-auto overscroll-contain">
              ...
              <select
                ...
                onChange={(event) =>
                  updateWritingMode.mutate({
                    docId,
                    writingMode: event.target.value as WritingMode,
                  })
                }
                className={[
            ...
            <section className="pt-5">
              <p className="font-mono-label mb-1 px-6 pb-3 text-[10px] tracking-[0.2em] text-(--w-subtle) uppercase">
                Actions
              </p>
