...
const TOOLBAR_BUTTON_WIDTH = 24;
const TOOLBAR_BUTTON_GAP = 20;
...
export default function TiptapMenuBar({ editor, aiEnabled, onAiOpen }: Props) {
  ...
  const buttons: ToolbarButton[] = [
    ...
    {
      label: <i className="font-display text-sm italic">I</i>,
      title: "Italic",
  ...
  return (
    <div
      ...
      inert={!isVisible}
    >
      <div className="pointer-events-auto flex h-[46px] items-center gap-5 border border-(--w-foreground) bg-(--w-background) px-3.5">
        {buttons.map((button) => (
          <button
            key={button.title}
            type="button"
            ...
            className={`flex size-6 cursor-pointer items-center justify-center border-0 transition-opacity hover:opacity-50 ${
              button.title === "Ask AI"
                ? "font-mono-label w-auto bg-(--w-foreground) px-2.5 text-[10px] tracking-[0.14em] text-(--w-background)"
                : button.isActive
                  ? "bg-(--w-foreground) text-(--w-background)"
                  : "bg-transparent text-(--w-foreground)"
            }`}
          >
            {button.label}
          </button>
