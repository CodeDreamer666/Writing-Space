...
export default function DocsHome() {
    ...
    const { data: deletedDocs } = api.docs.getDeletedDocs.useQuery(undefined, {
        enabled: isAuthenticated,
        retry: false,
    });
    ...
    const deleteDoc = api.docs.deleteDoc.useMutation({
        ...
        onSettled: async () => {
            await Promise.all([
                utils.docs.getUserDocs.invalidate(),
                utils.docs.getDeletedDocs.invalidate(),
            ]);
        },
    });

    const restoreDoc = api.docs.restoreDoc.useMutation({
        onError: (mutationError) => {
            handleTRPCError({ error: mutationError, router });
        },
        onSettled: async () => {
                                ...
                                <div className="relative overflow-hidden rounded-xl border border-dashed border-[var(--w-border-soft)] bg-[linear-gradient(135deg,color-mix(in_srgb,var(--w-border-soft)_32%,transparent),transparent_52%)] px-5 py-11 text-center sm:py-12">
                                    ...
                                    <div className="relative mx-auto flex flex-col items-center">
                                        ...
                                        <button
                                            ...
                                            className="mt-4 w-full inline-flex min-h-10 cursor-pointer items-center justify-center rounded-lg bg-[var(--w-foreground)] px-4 text-sm font-medium text-[var(--w-background)] transition-all duration-200 hover:opacity-85 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--w-foreground)] active:scale-[0.98] disabled:cursor-wait disabled:opacity-60"
                                        >
