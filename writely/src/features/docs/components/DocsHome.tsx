...
import {
  ...
  hasPendingDraft,
  queuePendingDraft,
  readPendingDraftText,
} from "../utils/pendingDraft";
...
export default function DocsHome() {
  ...
  const createDocument = useCallback(
    (initialText?: string) => {
      ...
      mutateCreateDoc(initialText ? { initialText } : undefined);
    },
    [mutateCreateDoc],
  );
  ...
  useEffect(() => {
    ...
    const pendingDraftText = readPendingDraftText();

    if (pendingDraftText === null) {
      showMessageRef.current(
        "Writely couldn't recover your starting sentence from this browser.",
        false,
      );
      return;
    }

    createDocument(pendingDraftText);
  ...
  const startGoogleSignIn = async (createDraftAfterSignIn: boolean) => {
    ...
    if (createDraftAfterSignIn) {
      if (!queuePendingDraft("")) {
        signInRequestRef.current = false;
        setIsSigningIn(false);
        showMessage(
          "Writely couldn't safely prepare your draft in this browser. Please try again.",
          false,
        );
        return;
      }
        ...
        <section className="px-6 pt-8 pb-12 sm:px-8 sm:pt-12">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="min-h-12 cursor-pointer rounded-xl bg-(--w-foreground) px-6 text-sm font-medium text-(--w-background) transition-all duration-200 hover:opacity-85 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-(--w-foreground) active:scale-[0.98] disabled:cursor-wait disabled:opacity-60"
            >
        ...
        {isAuthenticated && (
          <>
            ...
            >
              ...
              {!docs || docs.length === 0 ? (
                <div className="relative overflow-hidden rounded-xl border border-dashed border-(--w-border-soft) bg-[linear-gradient(135deg,color-mix(in_srgb,var(--w-border-soft)_32%,transparent),transparent_52%)] px-5 py-11 text-center sm:py-12">
                  <div
                    aria-hidden="true"
                    className="pointer-events-none absolute inset-0 bg-[radial-gradient(var(--w-border-soft)_1px,transparent_1px)] mask-[linear-gradient(to_bottom,black,transparent)] bg-size-[14px_14px] opacity-70"
                  />

                  <div className="relative mx-auto flex flex-col items-center">
                    <div className="flex justify-center gap-4">
                      <div className="relative flex size-16 items-center justify-center rounded-2xl border border-(--w-border) bg-(--w-background) text-(--w-muted) shadow-[0_12px_30px_color-mix(in_srgb,var(--w-background)_30%,transparent)]">
                        <svg
                          viewBox="0 0 24 24"
                          fill="none"
                          stroke="currentColor"
                    ...
                    <button
                      ...
                      className="mt-4 inline-flex min-h-10 w-full cursor-pointer items-center justify-center rounded-lg bg-(--w-foreground) px-4 text-sm font-medium text-(--w-background) transition-all duration-200 hover:opacity-85 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-(--w-foreground) active:scale-[0.98] disabled:cursor-wait disabled:opacity-60"
                    >
                  ...
                  {docs.map(({ title, id, updatedAt }) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="mr-2 flex size-11 cursor-pointer items-center justify-center rounded-lg text-(--w-muted) hover:bg-(--w-border-soft) hover:text-(--w-foreground) focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--w-muted)"
                      >
                        <svg
                          viewBox="0 0 24 24"
                          fill="none"
