    ...
    return (
        <>
            ...
            <aside
                ...
                aria-hidden={!isOpen}
                inert={!isOpen}
                className={`fixed inset-y-0 right-0 z-50 flex h-dvh w-[min(100%,420px)] transform-gpu flex-col overflow-hidden border-l border-[var(--w-border)] bg-[var(--w-surface)]/98 shadow-[-24px_0_80px_rgba(0,0,0,0.48)] backdrop-blur-xl transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] will-change-transform ${isOpen ? "translate-x-0" : "pointer-events-none translate-x-full"
                    }`}
            >
