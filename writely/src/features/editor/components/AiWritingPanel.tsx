...
export default function AiWritingPanel({
  ...
  const runAction = (action: AiAction, instruction?: string) => {
    ...
    if (!target?.trim()) {
      setRequestError({
        message: "Select the text you want Writely AI to work on.",
        selectionVersion,
        panelVersion,
      });
      return;
    ...
    if (target.length > MAX_AI_SELECTION_CHARACTERS) {
      setRequestError({
        message: `AI selections can contain up to ${MAX_AI_SELECTION_CHARACTERS.toLocaleString("en")} characters.`,
        selectionVersion,
        panelVersion,
      });
      return;
